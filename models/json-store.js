'use strict';
//general file for managing databaes within the project, but we are only  gonna use 2 methods for now
...
class JsonStore {
  ...
  findOneBy(collection, filter) {// and this one
    const results = this.db.data[collection].filter(filter);
    return results[0];
  }
